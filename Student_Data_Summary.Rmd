...
packages=c("tidyverse", "ggplot2", "mclust", "rpart", "rpart.plot", "arules", "caret")
...
for (col in names(train)[numeric_columns]) {
  train[[col]] <- discretize(train[[col]], method = "interval", breaks = 5)
}

transaction_id  <- as.character(train[["transid"]])
train$transid <- NULL
...
#rules <- apriori(transactions, parameter = list(supp = 0.2, conf = 0.8, minlen = 2))
...
# Inspect the top rules of Dropout
#inspect(head(sort(rules,decreasing = TRUE,  by = "lift"), 5))
...
I can not make this model produce meaningful results.
```{r}
library(mclust)
sum(is.na(train))

train_km <- select(train, !c())
modelName = 'VVI'
emdata = train
z = unmap(train[['Target']])
emdata_num <- sapply(emdata, as.numeric)
emdata_num_complete <- emdata_num[complete.cases(emdata_num), ]
completez <- z[complete.cases(emdata_num), ]
msEst <- mstep(emdata_num_complete, 'EEE', completez)
modelName = msEst$modelName
parameters = msEst$parameters
em_model <- em(emdata, modelName, parameters)
summary(em_model)
emclusters <- apply(em_model[["z"]], 1, which.max)
emclusters
 emdata$cluster
 emdata$Target <- train$Target
...
test$Target <- NA
predictions <- predict(tree_model, test[, !names(test) %in% "Target"], type = "class")
# GETTING ERROR WHILE TESTING THE MODEL. IF THIS IS CLEARED, WE ARE GOOD TO GO!
